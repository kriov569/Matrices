        ...
        private static int determinant(int[][] matrix) {
            ...
            for (int i = 0; i < matrix.length; i++) {
                ...
                for (int j = 1; j < matrix.length; j++) {
                    System.arraycopy(matrix[j], 0, smallerMatrix[j - 1], 0, i);
                    System.arraycopy(matrix[j], i + 1, smallerMatrix [j - 1], i, matrix.length - i - 1);// Заполнение smallerMatrix нужными значениями из matrix
                }
